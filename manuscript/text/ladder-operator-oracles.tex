...
\begin{equation}
    \begin{split}
        ...
        \ket{q_{I_a - 1}^{w-1} \hdots q_{I_a - 1}^{1} q_{I_a - 1}^0} \otimes \hdots \otimes \\
        \ket{q_{1}^{w-1} \hdots q_{1}^{1} q_{1}^0} \otimes \ket{q_{0}^{w-1} \hdots q_{0}^{1} q_{0}^0} \\
    \end{split}
\end{equation}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
where the phase flip induced by the parity of the occupation of the preceeding fermionic modes is omitted for brevity.
\ws{Comment on action on superposition states}
...
\begin{equation}
    \begin{split}
        (b_j b_l - b_j^\dagger b_l^\dagger) \ket{\psi_{b_l} \psi_{b_j}} = \begin{cases} 
            - \ket{11} & \text{when $\ket{\psi_{b_l} \psi_{b_j}}$ is $\ket{00}$} \\
            \hspace{1em}\ket{00} & \text{when $\ket{\psi_{b_l} \psi_{b_j}}$ is $\ket{11}$} \\
                                        \end{cases}
    \end{split}
\end{equation}
...
\ws{Add cases where parity is odd}
