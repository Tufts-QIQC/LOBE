...
\ws{restriction to unitary operators}
...
\ws{reorganize to be definitions then statement, check order of oracle definitions in camps and lin}
\begin{theorem}
...
The factor of $2^n$ in the rescaling factor (Eq. \ref{eq:rescaling-so}) comes from the sparsity of the matrix and the factor $\max_{ij} {|A_{ij}|}$ comes from the constraint in Theorem \ref{th:sparse-oracles} that all elements of $A$ have magnitude at most $1$.
...
\ws{what did i mean by analogous?}
...
Explicitly compiled Sparse-Oracle block-encodings of the form given in Eq. \ref{eq:so-be} for particular systems have been explored in several works \cite{camps2022fable, liu2024efficient, sanavio2024explicit} \ws{add citations here}.
...
\end{equation}
where $\alpha_l$ are \ws{positive} real-valued coefficients and each $U_l$ is a unitary operator.
...
Assuming that $A$ is sparse in this formulation - $L$ grows polynomially with $N_A$ - then the rescaling factor for a Sparse-Oracle block-encoding would be $\lambda_{SO} = 2^{\lceil \log_2 L \rceil} \max_{l} |\alpha_l|$.
...
Another way to frame this is creating an LCU block-encoding where the unitary operators themselves are block-encodings of operators which is described in Section 4.3 of Gilyén et. al \cite{gilyen2019quantum} and noted by Jennings et. al \cite{jennings2023efficient}. \ws{check if also in lin lin}
...
Block-encodings for a product of operators can be produced using a product of the block-encodings for each operator \cite{gilyen2019quantum}:
\begin{theorem}
    \label{th:product}
