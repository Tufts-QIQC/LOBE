...
# validation, bosonic_rotation, clean_ancilla, index, occupancy_1, occupancy_0
...
def get_bosonic_select_oracle_test_inputs():
    ...
    clean_ancilla = [cirq.LineQubit(i + 3) for i in range(2)]
    ...
    initial_state_of_clean_ancilla = np.zeros(1 << 4)
...
def test_select_oracle_on_one_two_body_bosonic_terms(
    ...
    clean_ancilla = [
        ...
        for i in range(number_of_clean_ancilla)
    ]
    index_register = [
        cirq.LineQubit(
            i + number_of_clean_ancilla + 1 + maximum_number_of_bosonic_ops_in_term
        )
    ...
    clean_ancilla_state = np.zeros(
        1 << (number_of_clean_ancilla + maximum_number_of_bosonic_ops_in_term)
    )
