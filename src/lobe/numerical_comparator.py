...
def is_less_than(qubits, reference, qbool, clean_ancillae, uncompute=True):
    ...
    if reference == 1:
        gates.append(
            cirq.Moment(
                cirq.X.on(qbool).controlled_by(
                    *qubits, control_values=[0] * len(qubits)
                )
            )
        )
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(qbool).controlled_by(
                    *qubits[:-1], control_values=[0] * (len(qubits) - 1)
                )
            )
        )
        return gates, qbool, 0
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[0]).controlled_by(
                    *qubits[:-2], control_values=[0] * (len(qubits) - 2)
                )
            )
        )
        gates.append(
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(qbool).controlled_by(
                    clean_ancillae[0], clean_ancillae[1], control_values=[1, 0]
                )
            )
        )
        gates += gates[:-1][::-1]
            ...
            gates.append(
                cirq.Moment(
                    cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                        *qubits[:-3], control_values=[0] * (len(qubits) - 3)
                    )
                )
            )  # is less than 8
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                    *qubits[-3:-1], control_values=[1, 1]
                )
            )
        )  # is equal to 6 or 7
        gates.append(cirq.Moment(cirq.X.on(clean_ancillae[ancillae_counter])))
        ctrls[0].append(clean_ancillae[ancillae_counter])
        ctrls[1].append(1)
        ancillae_counter += 1
        gates.append(
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(*qubits[-3:])
            )
        )  # is equal to 7
        gates.append(cirq.Moment(cirq.X.on(clean_ancillae[ancillae_counter])))
        ctrls[0].append(clean_ancillae[ancillae_counter])
        ...
        if len(qubits) > 4:
            gates.append(
                cirq.Moment(
                    cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                        *qubits[:-4], control_values=[0] * (len(qubits) - 4)
                    )
                )
            )  # is less than 16
            ctrls[0].append(clean_ancillae[ancillae_counter])
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                    *qubits[-4:-1], control_values=[1, 0, 1]
                )
            )
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                    *qubits[-4:], control_values=[1, 0, 0, 1]
                )
            )
        )  # is equal to 9
        ctrls[0].append(clean_ancillae[ancillae_counter])
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                    *qubits[-4:], control_values=[1, 0, 1, 1]
                )
            )
        )  # is equal to 11
        ctrls[0].append(clean_ancillae[ancillae_counter])
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                    *qubits[-4:-1], control_values=[1, 1, 1]
                )
            )
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                    *qubits[-4:], control_values=[1, 1, 0, 1]
                )
            )
        )  # is equal to 13
        ctrls[0].append(clean_ancillae[ancillae_counter])
        ...
        gates.append(
            cirq.Moment(
                cirq.X.on(clean_ancillae[ancillae_counter]).controlled_by(
                    *qubits[-4:], control_values=[1, 1, 1, 1]
                )
            )
        )  # is equal to 15
        ctrls[0].append(clean_ancillae[ancillae_counter])
