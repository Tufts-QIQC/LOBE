...
def add_select_oracle(
    ...
    Args:
        ...
        system (System): An instance of the System class that holds the qubit registers storing the
            state of the system.
        operators (List[ParticleOperator]): The ladder operators included in the Hamiltonian.
    ...
    if isinstance(operators, ParticleOperator):
    ...
    circuit.append(
        cirq.Moment(
            cirq.X.on(clean_ancilla[0]).controlled_by(
                *system.bosonic_system[modes[0]],
                control_values=[0] * len(system.bosonic_system[modes[0]]),
            )
        )
    )
