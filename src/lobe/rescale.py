...
def rescale_coefficients(terms, coefficients, maximum_occupation_number):
    """Rescale the coefficients of the terms to work with LOBE
    ...
    for i, coeff in enumerate(coefficients):
        ...
        rescaled_coefficients.append((1 / scale) * coeff)

    scaling_factor = (maximum_occupation_number + 1) ** (
        (max(numbers_of_bosonic_ops)) / 2
    )

    return rescaled_coefficients, scaling_factor
